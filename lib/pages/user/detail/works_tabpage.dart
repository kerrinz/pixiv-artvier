...
class WorksTabPage extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<ConsumerStatefulWidget> createState() => WorksTabPageState();
}

class WorksTabPageState extends ConsumerState<WorksTabPage> with AutomaticKeepAliveClientMixin {
  /// 列表数据统一管理
  ...
  Future<CommonIllustList> requestIllusts(WorksType worksType) async {
    ...
    resetLazyload();
    _cancelToken = CancelToken();
    return await ApiUser(ref.read(httpRequesterProvider)).getUserIllusts(
  ...
  Future<CommonNovelList> requestNovels() async {
    resetLazyload();
    _cancelToken = CancelToken();
    return await ApiUser(ref.read(httpRequesterProvider)).getUserNovels(
      userId: widget.userId,
      cancelToken: _cancelToken,
    );
  }
