...
import 'package:pixgem/config/enums.dart' as enums;
...
/// TODO: 需要全部重构
class CollectionsTabPage extends ConsumerStatefulWidget {
  final String userId;
  ...
  @override
  ConsumerState<ConsumerStatefulWidget> createState() => CollectionsTabPageState();
}

class CollectionsTabPageState extends ConsumerState<CollectionsTabPage> with AutomaticKeepAliveClientMixin {
  /// 列表数据统一管理
  ...
  void defaultIllustLazyload(WorksType worksType) {
    ...
    ApiIllusts(ref.read(httpRequesterProvider)).getNextIllusts(nextUrl!, cancelToken: _cancelToken).then((value) {
  ...
  void defaultNovelLazyload() async {
    ...
    _lazyloadProvider.setLazyloadStatus(LazyloadStatus.loading);
    _cancelToken = CancelToken();
    ApiNovels(ref.read(httpRequesterProvider)).nextNovels(nextUrl!, cancelToken: _cancelToken).then((value) {
  ...
  Future<CommonIllustList> requestIllusts(String restrict) async {
    resetLazyload();
    _cancelToken = CancelToken();
    return await ApiUser(ref.read(httpRequesterProvider)).artworkCollections(
      userId: widget.userId,
      restrict: restrict == "public" ? enums.Restrict.public : enums.Restrict.private,
  ...
  Future<CommonNovelList> requestNovels() async {
    resetLazyload();
    _cancelToken = CancelToken();
    return await ApiUser(ref.read(httpRequesterProvider)).novelCollections(
      userId: widget.userId,
      cancelToken: _cancelToken,
    );
  }
